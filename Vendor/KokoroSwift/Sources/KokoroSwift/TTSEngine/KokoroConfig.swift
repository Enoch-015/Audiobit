...
struct KokoroConfig: Decodable {
  ...
  nonisolated static func loadConfig() -> KokoroConfig {
    // Finder apps keep SwiftPM resource bundles under Contents/Resources.
    let packagedBundle = Bundle.main.resourceURL
      .map { $0.appendingPathComponent("DocumentReader_KokoroSwift.bundle") }
      .flatMap(Bundle.init(url:))
    let fileURL = (packagedBundle ?? Bundle.module)
      .url(forResource: "config", withExtension: "json", subdirectory: "Resources")!
